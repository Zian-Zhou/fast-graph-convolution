...
def load_data(dataset_str, val_size):
    """Load data."""
    names = ['x', 'y', 'tx', 'ty', 'allx', 'ally', 'graph']
    objects = []
    for i in range(len(names)):
    ...
    #################################################
    #'''
    #### 500 val set  默认为500  命令行修改
    idx_train = range(len(ally)-val_size)
    idx_val = range(len(ally)-val_size, len(ally))
    '''
    idx_train = range(len(ally)-2000)
    idx_val = range(len(ally)-2000, len(ally))
    '''
    #################################################    
...
def construct_feed_dict_v2(features, support, labels, labels_mask, placeholders):
    """Construct feed dictionary. 
    for highway network which has 2 branches
    """
    feed_dict = dict()
    feed_dict.update({placeholders['labels']: labels})
    feed_dict.update({placeholders['labels_mask']: labels_mask})
