...
class Dense(Layer):
    ...
    def _call(self, inputs):
        ...
        if self.sparse_inputs:
            ...
            x = tf.nn.dropout(x, rate = self.dropout)########## x = tf.nn.dropout(x, 1-self.dropout)
...
class CarryGate(Layer):
    """Dense layer."""
    def __init__(self, input_dim, output_dim, placeholders, dropout=0., sparse_inputs=False,
                 act=tf.nn.sigmoid, bias=True, featureless=False, **kwargs):
        super(CarryGate, self).__init__(**kwargs)
        ...
        with tf.variable_scope(self.name + '_vars'):
            self.vars['weights'] = glorot([input_dim, output_dim],
                                          name='weights')
            if self.bias:
                # 使网络初始倾向于carry features，即保留自身特征信息
                #self.vars['bias'] = positive_uniform([output_dim], name='bias')
                #self.vars['bias'] = zeros([output_dim], name='bias')
                self.vars['bias'] = negative_uniform([output_dim], name='bias')
