...
class MLP_GCN_HIGHWAY_MIX_v2(BasicModule):
	def __init__(self, placeholders, input_dim, config, **kwargs):
		super(MLP_GCN_HIGHWAY_MIX_v2, self).__init__(**kwargs)
	...
	def build(self):
		...
		self.gate_activations.append(self.mlp_inputs)
		...
		self.activations.append( 
						self.mlp_activations[-1] + self.gcn_activations[-1]
					 )
		'''
	...
	def _build(self):
		...
		'''
		self.gate_layers.append(
			CarryGate(input_dim = self.input_dim,
					output_dim = self.config.hidden1,#1
					placeholders = self.placeholders,
					act = tf.nn.sigmoid,
					bias = True,#True
